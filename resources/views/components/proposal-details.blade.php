...
<div class="card border-0 shadow-sm rounded-4 mb-4">
    <div class="card-body p-5">
        <div class="d-flex justify-content-between align-items-start mb-4">
            ...
            @if ($isAdmin)
                <div class="text-end text-muted fs-7">
                    Submitted by:<br>
                    <strong class="text-dark">{{ $proposal->user->name ?? 'Unknown User' }}</strong><br>
        ...
        <div class="bg-light border rounded-3 p-3 mb-4">
            <div class="fw-bold text-dark mb-2"><i class="bi bi-person-badge me-2 text-danger"></i> Primary Contact</div>
            <div class="row g-2">
                <div class="col-md-4"><span class="text-muted fs-7 d-block">Name:</span><span
                        class="fw-semibold">{{ $proposal->contact_name }}</span></div>
                <div class="col-md-4"><span class="text-muted fs-7 d-block">Email:</span><a
                        href="mailto:{{ $proposal->contact_email }}"
                        class="fw-semibold text-decoration-none">{{ $proposal->contact_email }}</a></div>
                <div class="col-md-4"><span class="text-muted fs-7 d-block">Phone:</span><a
                        href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $proposal->contact_phone) }}"
                        target="_blank" class="fw-semibold text-decoration-none text-success"><i
                            class="bi bi-whatsapp"></i> {{ $proposal->contact_phone }}</a></div>
            </div>
        </div>
        <hr class="my-4">
...
<div class="card border-0 shadow-sm rounded-4 mb-4">
    ...
    <div class="card-body p-4 pt-0">
        ...
        @if ($proposal->request_type === 'Fresh Money Funding')
            <div class="alert alert-success border-0 rounded-3 shadow-sm mb-0"><i
                    class="bi bi-check-circle-fill me-2"></i> This proposal requests financial sponsorship. See packages
                below.</div>
        @else
            <div>
...
@if ($proposal->packages)
    <div class="card border-0 shadow-sm rounded-4 mb-4 border-start border-danger border-4">
        <div class="card-header bg-white p-4 border-bottom-0 rounded-top-4">
            <h5 class="fw-bold text-dark mb-0"><i class="bi bi-box-seam text-danger me-2"></i> Sponsorship Packages</h5>
        </div>
        <div class="card-body p-4 pt-0">
            <div class="row g-3">
